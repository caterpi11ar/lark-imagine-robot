...
export class GeminiProvider implements ImageGenerationService {
  ...
  private static readonly RETRYABLE_CODES = [429, 500, 502, 503, 504]
  ...
  async generate(request: GenerationRequest): Promise<GenerationResult> {
    ...
    for (let attempt = 0; attempt <= GeminiProvider.MAX_RETRIES; attempt++) {
      try {
        ...
        if (attempt < GeminiProvider.MAX_RETRIES && this.isRetryable(msg)) {
          const delay = Math.min(1000 * 2 ** attempt, 16000)
          logger.warn(`Gemini request failed (attempt ${attempt + 1}/${GeminiProvider.MAX_RETRIES + 1}), retrying in ${delay}ms: ${msg}`)
  ...
  private isRetryable(errorMessage: string): boolean {
    return GeminiProvider.RETRYABLE_CODES.some(code =>
      errorMessage.includes(`"code": ${code}`) || errorMessage.includes(`"code":${code}`),
    )
  }
}
